...
telework_hh <- merged_data %>%
  ...
  group_by(serial) %>%
  summarize(full_remote = mean(full_remote),
...
merged_data %>%
  ...
  summarise(moved = weighted.mean(moved,w = asecwt, na.rm = TRUE))  
 
###########################
### Regression Analysis ###
###########################

merged_hh_data <- merged_data %>%
  left_join(telework_hh) %>%
  filter(asecwth > 0) %>%
...
merged_hh_data$household_remote_type <- factor(merged_hh_data$household_remote_type, levels = c("Not a Remote Household","Partially Remote Household","Fully Remote Household"))
  
# Convert moving variables to binary format within the merged_hh_data dataset
binary_moving_vars <- c("household_remote_type")

# Define the weights object for the feols
asecwth <- merged_hh_data$asecwth
...
all_results <- as.data.frame(c(bind_rows(any_baseline_models[[1]]$coeftable, any_indinfo_models[[1]]$coeftable,
                              in_county_baseline_models[[1]]$coeftable, in_county_indinfo_models[[1]]$coeftable,
                              out_county_baseline_models[[1]]$coeftable, out_county_indinfo_models[[1]]$coeftable)))
